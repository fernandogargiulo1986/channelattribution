...
RcppExport SEXP heuristic_models_cpp(SEXP Data_p, SEXP var_path_p, SEXP var_conv_p, SEXP var_value_p)
{
 ...
 if(flg_var_value==1){
  return List::create(
    Named("channel_name")=vchannels,
     Named("first_touch_conversions") = vfirst_conv,
      Named("first_touch_value") = vfirst_val,
       Named("last_touch_conversions") = vlast_conv,
        Named("last_touch_value") = vlast_val,
         Named("linear_touch_conversions") = vlinear_conv,
          Named("linear_touch_value") = vlinear_val);
 }else{
  return List::create(
    Named("channel_name")=vchannels,
     Named("first_touch") = vfirst_conv,
      Named("last_touch") = vlast_conv,
       Named("linear_touch") = vlinear_conv);
 }
